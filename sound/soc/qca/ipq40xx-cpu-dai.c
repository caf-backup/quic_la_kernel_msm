...
static int ipq40xx_i2s_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params,
					struct snd_soc_dai *dai)
{
	uint32_t bit_width, channels;
	uint32_t ret;
	uint32_t stereo_id = get_stereo_id(substream, I2S);
	uint32_t mbox_id = get_mbox_id(substream, I2S);
...
static int ipq40xx_i2s_prepare(struct snd_pcm_substream *substream,
					struct snd_soc_dai *dai)
{
	dev_dbg(dai->dev, "%s:%d\n", __func__, __LINE__);
	return 0;
...
static int ipq40xx_i2s_startup(struct snd_pcm_substream *substream,
					struct snd_soc_dai *dai)
{
	/* Check if the direction is enabled for the DMA/Stereo channel */
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if(dai_priv[I2S].tx_enabled != ENABLE)
			goto error;
	} else if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		if(dai_priv[I2S].rx_enabled != ENABLE)
			goto error;
	}
...
static void ipq40xx_i2s_shutdown(struct snd_pcm_substream *substream,
					struct snd_soc_dai *dai)
{
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
...
static int ipq40xx_i2s_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	dev_dbg(dai->dev, "%s:%d\n", __func__, __LINE__);
	return 0;
}

static int ipq40xx_tdm_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params,
					struct snd_soc_dai *dai)
{
	uint32_t bit_width, channels;
	uint32_t ret;
	uint32_t stereo_id = get_stereo_id(substream, TDM);
	uint32_t mbox_id = get_mbox_id(substream, TDM);
...
static int ipq40xx_tdm_prepare(struct snd_pcm_substream *substream,
					struct snd_soc_dai *dai)
{
	dev_dbg(dai->dev, "%s:%d\n", __func__, __LINE__);
	return 0;
...
static int ipq40xx_tdm_startup(struct snd_pcm_substream *substream,
					struct snd_soc_dai *dai)
{
	/* Check if the direction is enabled for the DMA/Stereo channel */
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if(dai_priv[TDM].tx_enabled != ENABLE)
			goto error;
	} else if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		if(dai_priv[TDM].rx_enabled != ENABLE)
			goto error;
	}
...
static void ipq40xx_tdm_shutdown(struct snd_pcm_substream *substream,
					struct snd_soc_dai *dai)
{
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ipq40xx_glb_tx_data_port_en(DISABLE);
...
static int ipq40xx_tdm_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	dev_dbg(dai->dev, "%s:%d\n", __func__, __LINE__);
	return 0;
}
static struct snd_soc_dai_ops ipq40xx_i2s_ops = {
	.startup	= ipq40xx_i2s_startup,
	.prepare	= ipq40xx_i2s_prepare,
	.hw_params	= ipq40xx_i2s_hw_params,
	.shutdown	= ipq40xx_i2s_shutdown,
	.set_fmt	= ipq40xx_i2s_set_fmt,
};

static struct snd_soc_dai_ops ipq40xx_tdm_ops = {
	.startup	= ipq40xx_tdm_startup,
	.prepare	= ipq40xx_tdm_prepare,
	.hw_params	= ipq40xx_tdm_hw_params,
	.shutdown	= ipq40xx_tdm_shutdown,
	.set_fmt	= ipq40xx_tdm_set_fmt,
};
