...
/**
 * wifi_diag_parse_args - parse input string into argc count and argv array
 *
 ...
 */
static int wifi_diag_parse_args(char *str, char **argv, int max_argc)
{
	int argc = 0;
	char c;
...
#define CALL_CMDH(cmdh)					\
	...
	data[count] = '\0';
	argc = wifi_diag_parse_args(data, argv, DEBUGFS_MAX_ARGC);
	if (argc < 2)
		return -EINVAL;
