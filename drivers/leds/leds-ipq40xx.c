...
struct qca961x_led_data {
	struct led_classdev cdev;
	int led_blink_idx;
};

static struct qca961x_led_data *leds;
		...
		enum led_brightness brightness)
{
	...
	qca961x_set_led_blink_set(led_cdev, &delay_on, &delay_off);
...
static int __exit qca961x_led_remove(struct platform_device *pdev)
{
	int i;
	if (leds) {
		for (i = 0; i < blink_idx_cnt; i++)
...
static struct platform_driver qca961x_led_driver = {
	.probe  = qca961x_led_probe,
	.remove = qca961x_led_remove,
	.driver = {
		.name   = LEDC_DRV_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = qca961x_match_table,
	},
};

module_platform_driver(qca961x_led_driver);
