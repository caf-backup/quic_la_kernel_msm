				      ...
				      struct switch_val *val)
{
	...
	for (i = 0; i < QCA_85XX_MAX_PORTS; i++)
		if ((ret = qca_85xx_sw_issue_mib_op(i, MIB_FLUSH)) != 0)
			return ret;
				    ...
				    struct switch_val *val)
{
	...
	if ((ret = qca_85xx_sw_issue_mib_op(port, MIB_CAPTURE)) != 0)
		return ret;

	for (i = 0; i < priv->num_mib_counters; i++) {
		counter_val = priv->read(MIB_RX_DATA + priv->mib[i].offset);
		if (priv->mib[i].size > 1) {
			counter_high = priv->read(MIB_RX_DATA + priv->mib[i].offset + 4);
...
static int qca_85xx_sw_hw_apply(struct switch_dev *dev)
{
	...
	for (i = 0; i < QCA_85XX_MAX_VLANS; i++) {
		if (priv->vlan_port_map[i] == 0)
			continue;
		ret = qca_85xx_port_vlan_group_enable(i, priv->vlan_port_map[i]);
		if (ret != 0)
			return ret;
	}
