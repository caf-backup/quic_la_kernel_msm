...
{
	if (!test_bit(ATH10K_FLAG_CRASH_FLUSH, &ar->dev_flags) ||
	    test_bit(ATH10K_FLAG_DEBUGFS_CRASH_INJECT, &ar->dev_flags))
...
int ath10k_snoc_fw_indication(struct ath10k *ar, u64 type)
{
	...
	if (test_bit(ATH10K_SNOC_FLAG_UNREGISTERED, &ar_snoc->flags))
		return 0;
...
static int ath10k_snoc_remove(struct platform_device *pdev)
{
	...
	if (test_bit(ATH10K_SNOC_FLAG_RECOVERY, &ar_snoc->flags))
		wait_for_completion_timeout(&ar->driver_recovery, 300 * HZ);

	set_bit(ATH10K_SNOC_FLAG_UNREGISTERED, &ar_snoc->flags);
