...
static int ath10k_htt_fetch_peer_stats_tlv(struct ath10k *ar,
					   struct sk_buff *skb)
{
	...
	while (len > 0) {
		...
		tlv_len = (HTT_LEN_ADDR_MASK & __le32_to_cpu(tlv->tag_length))
			  >> HTT_LEN_ADDR_BITS;
		ptr += sizeof(*tlv);
		len -= sizeof(*tlv);
		...
		if (tlv_len > len) {
			ath10k_dbg(ar, ATH10K_DBG_HTT,
				   "HTT tlv parse failure of tag %hu at byte %zd (%zu bytes left, %hu expected)\n",
		...
		if (test_bit(HTT_TX_STATS_VALID, &valid_bitmap)) {
			...
			ath10k_dbg(ar, ATH10K_DBG_HTT, "received invalid bitmap 0x%x for tag %hu at byte %zd\n",
...
bool ath10k_htt_t2h_msg_handler(struct ath10k *ar, struct sk_buff *skb)
{
	...
	case HTT_T2H_MSG_TYPE_PEER_STATS:
		if (ar->running_fw->fw_file.htt_op_version ==
			...
			ath10k_htt_fetch_peer_stats_tlv(ar, skb);
		else
