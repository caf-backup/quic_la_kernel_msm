...
{
	if ((skb->len > 3) &&
	   (skb->data[0] == QCA_DEBUG_MSG_MSB) &&
	   (skb->data[1] == QCA_DEBUG_MSG_LSB)) {
		bt_dev_dbg(hdev, " ACL QCA Debug packet");
		/* Send QCA Debug packet to diag as diag packet */
		hci_recv_diag(hdev, skb);
	} else {
		bt_dev_dbg(hdev, "ACL packet");
