...
static int qcom_ice_set_bus_bw(struct ice_device *ice_dev, uint32_t bw)
{
	int err = 0;

	if (bw != ice_dev->curr_bw) {
		err = icc_set(ice_dev->path, bw, 0);
		if (err) {
			dev_err(ice_dev->pdev, "%s(): failed to setbw err=%d\n",
				err);
			goto out;
		}
		ice_dev->curr_bw = bw;
...
static int qcom_ice_get_icc_path(struct ice_device *ice_dev)
{
	struct device *dev = ice_dev->pdev;
	int ret = 0;

	ice_dev->path = of_icc_get(dev, "cfg");
	if (IS_ERR(ice_dev->path))
		return PTR_ERR(ice_dev->path);
		...
		struct ice_device *ice_dev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "qcom,bandwidth-min",
			&ice_dev->min_bw))
		dev_err(dev, "minimum bw is not defined\n");

	if (of_property_read_u32(pdev->dev.of_node, "qcom,bandwidth-max",
			&ice_dev->max_bw))
		dev_err(dev, "minimum bw is not defined\n");
...
static int qcom_ice_finish_init(struct ice_device *ice_dev)
{
	...
	if (ice_dev->is_ice_clk_available) {
		...
		err = qcom_ice_get_icc_path(ice_dev);
		if (err)
			goto out;
	}
...
static int enable_ice_setup(struct ice_device *ice_dev)
{
	...
	ret = qcom_ice_set_bus_bw(ice_dev, ice_dev->max_bw);
	if (ret) {
		dev_err(ice_dev->pdev,
...
static int disable_ice_setup(struct ice_device *ice_dev)
{
	...
	ret = qcom_ice_set_bus_bw(ice_dev, ice_dev->min_bw);
	if (ret)
		dev_err(ice_dev->pdev, "%s():%pK: Unable to get bus bw %d\n",
			__func__, ice_dev, ret);
