...
static int q6v5_reset_assert(struct q6v5 *qproc)
{
	...
	if (qproc->has_alt_reset) {
		...
		reset_control_assert(qproc->pdc_reset);
		regmap_update_bits(qproc->tcsr_map, qproc->conn_box,
				   BIT(0), BIT(0));
				       ...
				       u32 offset)
{
	...
	/* Assert halt request */
	regmap_update_bits(halt_map, qproc->halt_nav,
			   NAV_AXI_HALTREQ_BIT, NAV_AXI_HALTREQ_BIT);
...
static int q6v5_init_mem(struct q6v5 *qproc, struct platform_device *pdev)
{
	...
	ret = of_parse_phandle_with_fixed_args(pdev->dev.of_node,
					       "qcom,halt-regs", 3, 1, &args);
	if (ret < 0) {
		dev_err(&pdev->dev, "failed to parse qcom,halt-regs\n");
		return -EINVAL;
	}

	qproc->halt_nav_map = syscon_node_to_regmap(args.np);
	of_node_put(args.np);
	if (IS_ERR(qproc->halt_nav_map))
		return PTR_ERR(qproc->halt_nav_map);

	qproc->halt_nav = args.args[0];

	ret = of_parse_phandle_with_fixed_args(pdev->dev.of_node,
					       "qcom,tcsr-regs", 1, 0, &args);
	if (ret < 0) {
		dev_err(&pdev->dev, "failed to parse qcom,halt-regs\n");
		return -EINVAL;
	}
	qproc->tcsr_map = syscon_node_to_regmap(args.np);
	of_node_put(args.np);
	if (IS_ERR(qproc->tcsr_map))
		return PTR_ERR(qproc->tcsr_map);
